...
import com.vitordepaula.invbrasil.databinding.ActivityScreenSaleRegisterBinding
...
class ScreenSaleRegister : AppCompatActivity() {

    private lateinit var binding: ActivityScreenSaleRegisterBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityScreenSaleRegisterBinding.inflate(layoutInflater)
        enableEdgeToEdge()
    ...
    private fun loadProduct(){
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main) {
                val adapter = ArrayAdapter(this@ScreenSaleRegister, android.R.layout.simple_spinner_item, names)
    ...
    private fun saveSale() {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (name.isNotBlank() && address.isNotBlank() && quantitySale > 0) {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@ScreenSaleRegister, "Venda Registrada com Sucesso", Toast.LENGTH_SHORT).show()
                    finish()
                }

            } else {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@ScreenSaleRegister, "Preencha Todos os Campos", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
